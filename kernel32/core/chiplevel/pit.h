/*
 * Header file for JTMOS PIT library.
 */
#ifndef __INCLUDED_PIT_H__
#define __INCLUDED_PIT_H__

//
#include "basdef.h"

//
#define IMPLEMENT_MULTIPLE_PIT_HANDLERS

// Max. amount of PIT handlers
#define N_MAX_PIT_HANDLERS	250

//
typedef struct
{
	// Number of handlers for PIT interrupt
	int n_handlers;

	// Pointers to handler functions
	void *handler[N_MAX_PIT_HANDLERS];

	// Estimations
	int secondInTicks;      // How many ticks is one second?
	int millisecondInTicks; // How many ticks is one millisecond?
}PIT;
extern PIT *pit;

//
extern void init_pit(long h, unsigned char channel);
extern unsigned int pit_getchannel(unsigned char channel);
