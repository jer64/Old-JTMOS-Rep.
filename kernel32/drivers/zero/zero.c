...
// check driver_api.h.
//
int zero_device_call(DEVICE_CALL_PARS)
{
 //
 DWORD *size;
...
// Registers device
int zero_register_device(void)
{
 // Check that the device isn't already registered
 if( driver_getdevicebyname("zero") )
 {
  print("cannot register device zero: device already registered");
  return 1;
 }

 // Register the device
 driver_register_new_device("zero",zero_device_call,DEVICE_TYPE_CHAR);
