...
// PTR to it.
SLPACKET *slipGetQueuedRecPacket(void)
{
	int i,record;
	SLPACKET *st,*sel;
...
void slipAddPacketToRecQueue(SLPACKET *p)
{
	int i;
	SLPACKET *st;
	WAITINGCLIENT *w;
...
void slipAddPacketToTraQueue(SLPACKET *p)
{
	int i;
	SLPACKET *st;
...
void slipCleanUpQueues(void)
{
	...
	for(i=0; i<N_MAX_QUEUED_SLIP_PACKETS; i++)
	{
		...
		{
			//
			if(debug)printf("packet time-out: removing queued recpacket %d\n",
				i);
...
void slipRecNewPackets(void)
{
	...
	if(sp.len)
	{
		...
		// Report
		if(debug)printf("%s: packet received, size=%d bytes\n",
			__FILE__, sp.len);
